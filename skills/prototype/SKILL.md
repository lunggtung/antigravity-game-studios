...
user-invocable: true
allowed-tools: view_file, find_by_name, grep_search, write_to_file, replace_file_content, run_command, invoke_subagent, ask_question
model: inherit
agent: prototyper
isolation: worktree
---
...
Otherwise, use `ask_question` to confirm intent before proceeding:
...
Assess which path best fits the hypothesis, then use `ask_question` with your
...
2. **Choose path** — same ask_question widget as Phase 3 (HTML / Engine / Paper).
