...
user-invocable: true
allowed-tools: view_file, find_by_name, grep_search, write_to_file, invoke_subagent, run_command
model: inherit
...
- **CONCERNS** → surface via `ask_question` with options: `Revise flagged rules` / `Accept and proceed` / `Discuss further`
