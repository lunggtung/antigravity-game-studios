...
user-invocable: true
allowed-tools: view_file, find_by_name, grep_search, write_to_file, run_command, ask_question
model: inherit
---
...
If a matching file is found, use `ask_question`:
...
Then use `ask_question` to present two options:
...
Run git log for the sprint period to understand what was actually committed and when. Use the run_command tool (which uses Git Bash on Windows — the `2>/dev/null` is bash syntax, not PowerShell):
