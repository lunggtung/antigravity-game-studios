...
user-invocable: true
allowed-tools: view_file, find_by_name, grep_search, write_to_file, run_command, ask_question, invoke_subagent
model: inherit
...
Display a one-paragraph summary of what the document will contain (layers, modules, data flows, ADR gaps). Then use `ask_question`:
...
Use `ask_question` — "Technical Director and Lead Programmer have reviewed the architecture. How would you like to proceed?"
...
Show the proposed Document Status block inline, then use `ask_question`:
...
5. **Use `ask_question` for write approvals** — plain text "May I?" is not
