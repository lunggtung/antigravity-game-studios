...
If [B]: display the complete proposed systems-index section, then re-ask with `ask_question`.
...
Use `ask_question` for the write approval:
...
Then close with `ask_question` tailored to the pre-gate checklist state:
...
3. **Offer options** via ask_question with three choices:
