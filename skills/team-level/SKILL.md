...
user-invocable: true
allowed-tools: view_file, find_by_name, grep_search, write_to_file, replace_file_content, run_command, invoke_subagent, ask_question, write_to_file
model: inherit
---

When this skill is invoked:

**Decision Points:** At each step transition, use `ask_question` to present
...
Use the invoke_subagent tool to spawn each team member as a subagent:
...
**Gate**: Use `ask_question` to present all three Step 1 outputs (narrative brief, lore foundation, visual direction targets) and confirm before proceeding to Step 2.
...
**Gate**: Use `ask_question` to present Step 2 layout (including any unresolved adjacent area dependencies) and confirm before proceeding to Step 3.
...
**Gate**: Use `ask_question` to present Step 3 outputs and confirm before proceeding to Step 4.
...
**Gate**: Use `ask_question` to present both Step 4 results. If the accessibility-specialist returned any BLOCKING concerns, highlight them prominently and offer:
...
3. **Offer options** via ask_question with choices:
