...
user-invocable: true
allowed-tools: view_file, find_by_name, grep_search, write_to_file, WebSearch, invoke_subagent, ask_question, run_command
model: inherit
---
...
3. **Run through ideation phases** interactively, asking the user questions at
   ...
   **Use `ask_question`** at key decision points throughout brainstorming:
   ...
   `ask_question` to capture the decision with concise labels.
...
- What 3 games have you spent the most time with? What kept you coming back?
  *(Ask this as plain text — the user must be able to type specific game names freely.
  Do NOT put this in an ask_question with preset options.)*
- Are there genres you love? Genres you avoid? Why?
- Do you prefer games that challenge you, relax you, tell you stories,
  or let you express yourself? *(Use `ask_question` for this — constrained choice.)*

**Practical constraints** (shape the sandbox before brainstorming).
Bundle these into a single multi-tab `ask_question` with these exact tab labels:
...
Present all three. Then use `ask_question` to capture the selection.
...
Ask these as `ask_question` calls — derive the options from the chosen concept, don't hardcode them:
...
**Pillar confirmation**: After presenting the full pillar set, use `ask_question`:
...
If the user selects B, C, or D, make the revision, then use `ask_question` again:
...
Collect both verdicts, then present them together using a two-tab `ask_question`:
...
- **Target platform**: Use `ask_question` — "What platforms are you targeting for this game?"
...
- **Engine experience**: Use `ask_question` — "Do you already have an engine you work in?"
...
5. Use `ask_question` for write approval:
...
If [B]: ask which section to revise using `ask_question` with options: `Elevator Pitch` / `Core Fantasy & Unique Hook` / `Pillars` / `Core Loop` / `MVP Definition` / `Scope Tiers` / `Risks` / `Something else — I'll describe`

After revising, show the updated section as a diff or clear before/after, then use `ask_question` — "Ready to write the updated concept document?"
