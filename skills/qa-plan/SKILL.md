...
**Argument:** `$ARGUMENTS` (blank = ask user via ask_question)
...
then ask two questions together using `ask_question`:
...
- Use `ask_question` for scope selection when no argument is provided.
