...
user-invocable: true
allowed-tools: view_file, find_by_name, grep_search, ask_question, invoke_subagent, run_command
model: inherit
---
...
**Scope:** `$ARGUMENTS[0]` (blank = ask user via ask_question)
...
If no argument is given, use `ask_question`:
...
- **GAPS [list]** → surface the specific gaps to the user via `ask_question`:
