...
user-invocable: true
allowed-tools: view_file, find_by_name, grep_search, run_command, write_to_file, replace_file_content, ask_question, invoke_subagent
model: inherit
---
...
3. If multiple in-progress stories are found, use `ask_question`:
...
### Manual verification with confirmation (use `ask_question`)
...
Batch up to 4 manual verification questions into a single `ask_question` call:
...
1. Ask: is there a test — unit, integration, or confirmed manual playtest — that
   ...
   - **Manual confirmation**: if the criterion was verified via `ask_question`
...
- `lean` → use `ask_question` before proceeding:
...
Present the verdict to the user. If CONCERNS, surface them via `ask_question`:
...
Use `ask_question` before writing anything:
...
- Use `ask_question` for the code review prompt and for batching manual
