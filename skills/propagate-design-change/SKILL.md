...
user-invocable: true
allowed-tools: view_file, find_by_name, grep_search, write_to_file, run_command, invoke_subagent
model: inherit
...
- **CONCERNS** → surface the specific ADRs or recommendations flagged; use `ask_question` with options: `Revise the impact assessment` / `Accept with noted concerns` / `Discuss further`
