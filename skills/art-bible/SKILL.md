...
Use `ask_question` with two tabs:
...
Present the art-director's draft to the user. Use `ask_question`:
...
Collect both. If they conflict (e.g., art-director wants elaborate diegetic UI but ux-designer flags it would reduce combat readability), surface the conflict explicitly with both positions. Do NOT silently resolve — use `ask_question` to let the user decide.
...
Use `ask_question` for next steps. Only include options that are genuinely next based on the state check above:
