...
user-invocable: true
allowed-tools: view_file, find_by_name, grep_search, write_to_file, replace_file_content, ask_question, invoke_subagent, run_command
model: inherit
agent: ux-designer
---
...
**If no argument is provided**, do not fail — ask instead. Use `ask_question`:
...
2. **Questions**: Ask what is needed to draft this section. Use `ask_question`
...
3. **Options**: Where design choices exist, present 2-4 approaches with pros/cons.
   Explain reasoning in conversation, then use `ask_question` to capture the decision.
4. **Decision**: User picks an approach or provides custom direction.
5. **Draft**: Write the section content in conversation for review. Flag provisional
   assumptions explicitly.
6. **Approval**: Use `ask_question`:
...
7. **Write**: Use `ask_question`: "May I write the [section name] section to `[filepath]`?"
...
- Use `ask_question` to capture the choice:
...
- Use `ask_question`: "Want an ASCII wireframe as part of this spec?"
...
Use `ask_question` to step through items in groups of 3-4, not all at once.
...
Work through patterns in groups. Use `ask_question`:
...
When delegating to another agent via the invoke_subagent tool:
