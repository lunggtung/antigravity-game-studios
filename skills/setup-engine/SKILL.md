...
user-invocable: true
allowed-tools: view_file, find_by_name, grep_search, write_to_file, replace_file_content, WebSearch, WebFetch, invoke_subagent, ask_question, run_command
model: inherit
---
...
**Question 1 — Prior experience** (ask this first, always, via `ask_question`):
...
**Question 2 — Target platform** (ask this second, always, via `ask_question` — platform eliminates or heavily weights engines before any other factor):
...
5. Use `ask_question` to confirm: "Does this recommendation feel right, or would you like to explore a different engine?"
...
Use `ask_question` with concept-specific deep-dive topics. Always generate these options from the user's actual concept — do not use generic options. Always include at minimum:
...
### Remaining Sections
- **Performance Budgets**: Use `ask_question`:
