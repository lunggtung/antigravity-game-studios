...
5. If yes:
   ...
   - Append each missing section to the ADR file using the replace_file_content tool.
...
a **confirm/adjust** prompt using `ask_question` — not open-ended questions.
...
decisions remain, present each one as a separate `ask_question` with the proposed
...
5. **Write approval** — Use `ask_question`:
...
After the ADR is written (and registry optionally updated), close with `ask_question`.
