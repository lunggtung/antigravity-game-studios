...
user-invocable: true
allowed-tools: view_file, find_by_name, grep_search, run_command, write_to_file, invoke_subagent, ask_question
model: pro
---
...
> **Tool-action requirement**: At least 2 of the 5 challenge questions below must be answered by re-reading a specific file (view_file tool) or re-running a specific check (grep_search tool) — not by reflection alone. Mark these with [TOOL ACTION] to indicate a tool was used.
...
After the verdict is presented and any stage.txt update is complete, close with a structured next-step prompt using `ask_question`.
