...
user-invocable: true
allowed-tools: view_file, find_by_name, grep_search, write_to_file, replace_file_content, invoke_subagent, ask_question, write_to_file, run_command
model: inherit
---
...
2. If it exists: read it, find the highest-priority system with status "Not Started" or equivalent, and use `ask_question`:
...
6. If yes: proceed to **Phase 2 (Gather Context)** as normal, but in **Phase 3**
   ...
   incomplete sections.
7. **Never overwrite existing section content.** Use replace_file_content tool to replace only
...
2. **Questions**: Ask clarifying questions specific to this section. Use
   `ask_question` for constrained questions, conversational text for open-ended
   exploration.
...
3. **Options**: Where the section involves design choices (not just documentation),
   ...
   then use `ask_question` to capture the decision.
...
6. **Approval**: Immediately after the draft — in the SAME response — use
   `ask_question`. **NEVER use plain text. NEVER skip this step.**
...
7. **Write**: Use the replace_file_content tool to replace the placeholder with the approved content.
   **CRITICAL**: Always include the section heading in the `old_string` to ensure
   ...
   same placeholder and the replace_file_content tool requires a unique match. Use this pattern:
   ```
...
**Framing questions (ask BEFORE drafting)**: Use `ask_question` with a multi-tab widget:
...
**Framing question (ask BEFORE drafting)**: Use `ask_question`:
...
- Surface any disagreements between agents to the user via `ask_question`
...
- Present the specialists' proposals to the user for review via `ask_question`
...
**When delegating via invoke_subagent tool**:
...
- The main session presents the agent's output to the user via `ask_question`
- The user decides; the main session writes to file
...
2. **ask_question** at every decision point (Explain -> Capture pattern):
