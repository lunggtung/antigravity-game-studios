...
user-invocable: true
allowed-tools: view_file, find_by_name, grep_search, write_to_file, ask_question, run_command
model: inherit
---
...
This is the first thing the user sees. Use `ask_question` with these exact options so the user can click rather than type:
...
1. Ask them to describe their concept in one sentence — genre and core mechanic. Use plain text, not ask_question (it's an open response).
2. Acknowledge the concept, then use `ask_question` to offer two paths:
...
**If it does not exist**: Use `ask_question`:
...
After presenting the recommended path, use `ask_question` to ask the user which step they'd like to take first. Never auto-run the next skill.
