...
user-invocable: true
allowed-tools: view_file, find_by_name, grep_search, write_to_file, invoke_subagent, ask_question, run_command
model: inherit
agent: qa-lead
---
...
Present the qa-lead's full strategy to the user, then use `ask_question`:
...
Use `ask_question` per story group (batched 3-4 at a time):
...
Batch stories into groups of 3-4 and use `ask_question` for each:
...
After each FAIL result: use `ask_question` to collect the failure description, then spawn `qa-tester` via Task to write a formal bug report in `production/qa/bugs/`.
