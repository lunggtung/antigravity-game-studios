...
user-invocable: true
allowed-tools: view_file, find_by_name, grep_search, write_to_file, replace_file_content, ask_question, write_to_file, invoke_subagent, run_command
model: inherit
---
...
Then use `ask_question` to capture feedback:
...
Use `ask_question` to ask: "Does this dependency ordering look right? Any
...
Use `ask_question` to ask: "Do these priority assignments match your vision?
...
Use `ask_question` for: "Start designing [system-name] now, pick a different
system, or stop here?"
...
After `/design-system` completes, use `ask_question`:
...
After the systems index is created (or after designing some systems), present next actions using `ask_question`:
